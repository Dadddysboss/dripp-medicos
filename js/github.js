...
export async function fetchFile(fileKey) {
  ...
  const path = `repos/${creds.owner}/${creds.repo}/contents/${encodeURIComponent(fileKey)}?ref=${encodeURIComponent(creds.branch)}`;
...
export async function commitFile(fileKey, dataObj, message) {
  ...
  async function putOnce(s) {
    ...
    const path = `repos/${creds.owner}/${creds.repo}/contents/${encodeURIComponent(fileKey)}`;
...
import { LS_KEYS as _LK, DEFAULT_PRODUCTS, DEFAULT_SALES, DEFAULT_DOCTORS } from './config.js';

export async function fetchAllFromGitHub() {
  const out = { products: null, sales: null, doctors: null, errors: [] };
  try {
