...
import { CATEGORIES, generateSaleId, fmtCurrency, baseUnitsFor, priceForUnit, maxQtyForUnit, unitLabel, formatPackagingShort, splitBaseUnits, getProductImageUrl, getFallbackImageSvg } from '../config.js';
...
export function renderPOSView() {
  ...
  return `
    <div class="pos-grid gap-4 lg:gap-5 safe-bottom">
      ...
      <aside class="min-w-0">
        <!-- Desktop Cart Panel -->
        <div class="hidden lg:block bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col h-[calc(100vh-7rem)] sticky top-24">
          ${renderCartPanel(controlledRequired)}
        </div>

        <!-- Mobile Cart Toggle Button -->
        <div class="lg:hidden fixed bottom-20 left-1/2 -translate-x-1/2 z-40">
          <button id="pos-cart-toggle" class="cart-toggle-btn flex items-center gap-2 px-4 py-3 rounded-full bg-gradient-to-r from-teal-600 to-teal-500 text-white font-semibold shadow-xl shadow-teal-500/30 touch-target"
                  aria-label="Open cart" aria-expanded="false">
            ...
            <span id="pos-mobile-item-count" class="hidden sm:inline">${posCart.length} item${posCart.length === 1 ? '' : 's'}</span>
            <span class="w-5 h-5 flex items-center justify-center bg-white/20 rounded-full text-xs font-bold">${posCart.length}</span>
          </button>
        </div>

        <!-- Mobile Cart Drawer -->
        <div id="pos-cart-drawer-backdrop" class="cart-drawer-backdrop lg:hidden fixed inset-0 bg-black/50 z-40" aria-hidden="true"></div>
        <div id="pos-cart-drawer" class="cart-drawer lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-slate-900 border-t-2 border-teal-500 rounded-t-2xl shadow-2xl max-h-[85vh] flex flex-col" role="dialog" aria-label="Shopping cart" aria-hidden="true">
          <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800 sticky top-0 bg-white dark:bg-slate-900 rounded-t-2xl">
            <h3 class="text-lg font-bold flex items-center gap-2">
              <svg class="w-5 h-5 text-teal-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.7 13.4a2 2 0 0 0 2 1.6h9.7a2 2 0 0 0 2-1.6L23 6H6"/>
              </svg>
              Current Bill
            </h3>
            <button id="pos-cart-drawer-close" class="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 touch-target" aria-label="Close cart">
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-2" id="pos-mobile-cart">
            ${posCart.length === 0 ? renderEmptyCart() : posCart.map(renderCartRow).join('')}
          </div>
          <div class="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 sticky bottom-0 rounded-b-2xl safe-bottom">
            ${renderCartSummary(controlledRequired)}
          </div>
        </div>
      </aside>
    </div>

    ${renderCartPanelMarkup(controlledRequired)}
    ${renderCartSummaryMarkup(controlledRequired)}
...
export function mountPOS() {
  ...
  document.getElementById('pos-search')?.addEventListener('input', (e) => { 
    session.query = e.target.value; 
    
    // Debounce search to check for out-of-stock items and show substitutes
    clearTimeout(searchDebounce);
    searchDebounce = setTimeout(() => {
      checkForOutOfStockAndShowSubstitutes(e.target.value);
    }, 300);
  ...
  const toggleBtn = document.getElementById('pos-cart-toggle');
  const drawer = document.getElementById('pos-cart-drawer');
  const backdrop = document.getElementById('pos-cart-drawer-backdrop');
  const closeBtn = document.getElementById('pos-cart-drawer-close');
  ...
  // Mobile cart events (delegated on mobile cart container)
  document.getElementById('pos-mobile-cart')?.addEventListener('click', (e) => {
    const inc = e.target.closest('[data-cart-inc]');
    const dec = e.target.closest('[data-cart-dec]');
    const rm  = e.target.closest('[data-cart-rm]');
    if (inc) return incCart(inc.dataset.cartInc, +1);
  ...
  document.getElementById('pos-mobile-cart')?.addEventListener('change', (e) => {
    const inp = e.target.closest('[data-cart-qty]');
    if (!inp) return;
    setCartQty(inp.dataset.cartQty, inp.value);
  });
...
function findGenericSubstitutes(outOfStockProduct) {
  const products = Array.isArray(state.products) ? state.products : [];
  const salt = (outOfStockProduct.genericName || '').trim().toLowerCase();
  if (!salt) return [];
  
  return products.filter(p => {
    const pSalt = (p.genericName || '').trim().toLowerCase();
    const stock = Math.max(0, parseInt(p.totalBaseUnits, 10) || 0);
    const status = getProductStatus(p);
    return pSalt === salt && stock > 0 && !status.isExpired && p.id !== outOfStockProduct.id;
  });
}

function openSubstituteFinderModal(product) {
  const substitutes = findGenericSubstitutes(product);
  if (substitutes.length === 0) {
    showToast(`No in-stock alternatives found for ${product.genericName || product.name}`, 'warn');
    return;
  }
  ...
  showModal(`
    <div class="p-6 max-w-md">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-bold">Generic Alternatives for ${escapeHtml(product.name)}</h3>
        <button data-close class="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800">
          <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        </button>
      </div>
      <p class="text-sm text-slate-600 dark:text-slate-400 mb-4">Active salt: <strong class="text-slate-900 dark:text-slate-100">${escapeHtml(product.genericName || 'Unknown')}</strong> — Select an in-stock alternative:</p>
      <div class="space-y-2 max-h-80 overflow-y-auto">
        ${substitutes.map(s => {
          const stock = Math.max(0, parseInt(s.totalBaseUnits, 10) || 0);
          const split = splitBaseUnits(stock, s);
          const status = getProductStatus(s);
          return `
            <button data-substitute="${escapeAttr(s.id)}"
                    class="w-full text-left p-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:border-teal-400 dark:hover:border-teal-500 transition-colors">
              <div class="font-medium text-slate-900 dark:text-slate-100">${escapeHtml(s.name)}</div>
              <div class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">${escapeHtml(s.genericName || '')}</div>
              <div class="mt-1 flex flex-wrap gap-1 text-[10px]">
                <span class="px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300 font-semibold">${formatPackagingShort(Math.max(0, parseInt(s.totalBaseUnits, 10) || 0), s)}</span>
                ${s.isColdChain ? '<span class="px-1.5 py-0.5 rounded bg-sky-100 text-sky-700 dark:bg-sky-900/40 dark:text-sky-300">❄️</span>' : ''}
                ${s.isControlled ? '<span class="px-1.5 py-0.5 rounded bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300">🔒</span>' : ''}
              </div>
              <div class="mt-2 text-sm font-semibold text-teal-600 dark:text-teal-400">Box: ${fmtCurrency(s.boxUnitPrice)} ${s.stripUnitPrice ? ` | Strip: ${fmtCurrency(s.stripUnitPrice)}` : ''} ${s.tabletUnitPrice ? ` | Tab: ${fmtCurrency(s.tabletUnitPrice)}` : ''}</div>
            </button>
          `;
        }).join('')}
      </div>
  ...
  `, {
    ...
    onMount: (root) => {
      ...
      root.querySelectorAll('[data-substitute]').forEach(btn => {
        btn.addEventListener('click', () => {
          const subId = btn.dataset.substitute;
          const sub = state.products.find(p => p.id === subId);
          if (sub) {
            openUnitSelector(sub.id);
            hideModal();
          }
        });
      });
...
// --- Generic Substitute Finder for Search ---
function checkForOutOfStockAndShowSubstitutes(query) {
  if (!query || query.trim().length < 2) return;
  
  const products = Array.isArray(state.products) ? state.products : [];
  const q = query.trim().toLowerCase();
  
  // Find products matching the query that are out of stock
  const matchingProducts = products.filter(p => {
    const hay = `${p.name || ''} ${p.genericName || ''} ${p.batchNo || ''} ${p.rackNo || ''}`.toLowerCase();
    return hay.includes(q);
  });
  
  const outOfStockMatches = matchingProducts.filter(p => {
    const stock = Math.max(0, parseInt(p.totalBaseUnits, 10) || 0);
    return stock <= 0 || getProductStatus(p).isExpired;
  });
  
  // If exactly one out-of-stock match and user hasn't seen substitute modal recently
  if (outOfStockMatches.length === 1) {
    const product = outOfStockMatches[0];
    const lastShown = session.lastSubstituteShown || 0;
    const now = Date.now();
    if (now - lastShown > 5000) { // Show once every 5 seconds max
      session.lastSubstituteShown = now;
      openSubstituteFinderModal(product);
    }
  }
}
      ...
      <button data-add="${escapeAttr(p.id)}" ${disabled ? 'aria-disabled="true" tabindex="-1"' : ''}
        ...
        <div class="flex items-start justify-between gap-2">
          <div class="flex items-center gap-3 min-w-0">
            <div class="w-14 h-14 flex-shrink-0 rounded-lg bg-slate-100 dark:bg-slate-800 overflow-hidden flex items-center justify-center">
              <img src="${escapeAttr(imageUrl)}" alt="${escapeAttr(p.name || 'Medicine')}" class="w-full h-full object-cover" loading="lazy" onerror="this.src='${getFallbackImageSvg()}'"/>
            </div>
            <div class="min-w-0">
...
function openUnitSelector(productId) {
  ...
  showModal(`
    <div class="p-6 max-h-[90vh] overflow-y-auto" id="unit-modal">
  ...
  `, {
    size: 'lg',
    onMount: (root) => {
      // Event delegation on the modal root for all unit modal actions
      root.addEventListener('click', (e) => {
        // Quantity plus button
        const plusBtn = e.target.closest('[data-unit-inc]');
        if (plusBtn) {
          const unit = plusBtn.dataset.unitInc;
          const qtyInput = root.querySelector(`[data-unit-qty="${unit}"]`);
          const max = parseInt(qtyInput.max, 10) || 999;
          const current = parseInt(qtyInput.value, 10) || 1;
          qtyInput.value = Math.min(current + 1, max);
          return;
        }
        
        // Quantity minus button
        const minusBtn = e.target.closest('[data-unit-dec]');
        if (minusBtn) {
          const unit = minusBtn.dataset.unitDec;
          const qtyInput = root.querySelector(`[data-unit-qty="${unit}"]`);
          const current = parseInt(qtyInput.value, 10) || 1;
        ...
        if (addBtn) {
          const unit = addBtn.dataset.unitAdd;
          const qtyInput = root.querySelector(`[data-unit-qty="${unit}"]`);
...
function addToCart(productId, unit, qty) {
  ...
  showToast(`Added: ${product.name} × ${qty} ${unitLabel(unit)} — Saved & Synced with GitHub`, 'success', 1500);
...
function renderCartRow(it) {
  ...
  const imageUrl = product ? getProductImageUrl(product) : getFallbackImageSvg();
  return `
    <div class="flex items-center gap-2 p-2 rounded-lg bg-slate-50 dark:bg-slate-800/50">
      <div class="w-10 h-10 flex-shrink-0 rounded-lg bg-slate-100 dark:bg-slate-800 overflow-hidden flex items-center justify-center">
        <img src="${escapeAttr(imageUrl)}" alt="${escapeAttr(it.name)}" class="w-full h-full object-cover" onerror="this.src='${getFallbackImageSvg()}'"/>
      </div>
...
function handleCheckout() {
  ...
  rerenderCartAndSummary();
  showToast(`Sale ${saleId} completed — ${fmtCurrency(summary.netTotal)} — Saved & Synced with GitHub`, 'success', 2500);
}
...
// ============================================================
// 10. MOBILE CART HELPERS
// ============================================================

function renderCartPanel(controlledRequired) {
  const summary = computeSummary();
  return `
    <!-- Patient / Doctor / CNIC -->
...
function renderCartSummary(controlledRequired) {
  const summary = computeSummary();
  return `
    <!-- Patient / Doctor / CNIC (Mobile) -->
    <div class="px-4 py-3 border-b border-slate-200 dark:border-slate-800 grid grid-cols-2 gap-2">
      <div>
        <label class="block text-[10px] font-semibold uppercase tracking-wider text-slate-500 mb-1">Patient Name</label>
        <input id="pos-patient" type="text" value="${escapeAttr(session.patientName)}" placeholder="Optional"
...
function renderCartPanelMarkup(controlledRequired) {
  // Returns empty string since we render inline now
  return '';
}

function renderCartSummaryMarkup(controlledRequired) {
  // Returns empty string since we render inline now
  return '';
}
