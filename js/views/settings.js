...
import { fmtDateTime, LS_KEYS, DEFAULT_DOCTORS, DEFAULT_PRODUCTS, GEMINI_MODELS } from '../config.js';
...
import { loadImageLibrary, addImageToLibrary, removeImageFromLibrary, getProductImageUrl, getFallbackImageSvg } from '../config.js';
import { initAutoHealer, onTerminalLog, runHealthCheck, setAutoHealerEnabled } from '../services/autoHealer.js';
  ...
  return `
    <div class="space-y-6 max-w-5xl">
      ...
      <section class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
        <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div>
            <h2 class="text-base font-bold">AI &amp; API Configurations</h2>
            <p class="text-xs text-slate-500 dark:text-slate-400">Manage Google Gemini API key and active AI models for invoice OCR &amp; chatbot.</p>
          </div>
          <span class="text-[11px] uppercase tracking-wider px-2 py-1 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">
            Active
          </span>
        </div>
        <div class="p-5 space-y-4">
          <div>
            <label class="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Gemini API Key</label>
            <div class="relative">
              <input id="ai-apikey" type="password" value="${escapeAttr(getApiKey() || '')}" placeholder="AIzaSy..."
                     class="w-full px-3 py-2 pr-10 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm font-mono"/>
              <button id="ai-key-toggle" type="button" class="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded hover:bg-slate-100 dark:hover:bg-slate-700" title="Show / hide">
            ...
            <p class="mt-1 text-[11px] text-slate-500 dark:text-slate-400">Used for Gemini Flash OCR parsing of wholesale invoices and chatbot queries.</p>
          </div>
          <div>
            <label class="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Configured Gemini Models (Sequential Fallback Pipeline)</label>
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
              ${GEMINI_MODELS.map(m => `
                <div class="px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 text-xs font-mono flex items-center justify-between">
          ...
          <div>
            <button id="ai-save-key" class="px-4 py-2 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold shadow-md">Save Gemini API Key</button>
          </div>
        </div>
      </section>
      ...
      <section class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
        <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div>
            <h2 class="text-base font-bold">Medicine Image Library</h2>
            <p class="text-xs text-slate-500 dark:text-slate-400">Upload and manage product images. Select from library when adding/editing medicines.</p>
          </div>
          <button id="img-lib-upload" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-xs font-semibold">
        ...
        <div class="p-5">
          <input id="img-lib-file" type="file" accept="image/*" class="hidden"/>
          <div id="img-lib-grid" class="image-library-grid max-h-96 overflow-y-auto">
            ${renderImageLibrary()}
          </div>
        </div>
      </section>
...
export function mountSettings() {
  ...
  const terminalEl = document.getElementById('admin-terminal');
  const statusEl = document.getElementById('healer-status');
  const toggleEl = document.getElementById('healer-toggle');
  const clearBtn = document.getElementById('healer-clear');
  const healthCheckBtn = document.getElementById('healer-health-check');
  const exportLogsBtn = document.getElementById('healer-export-logs');
  ...
  function appendLog(message, type = 'info') {
    ...
    const prefix = colors[type] ? `<span class="${colors[type]}">[${type.toUpperCase()}]</span>` : `[${type.toUpperCase()}]`;
    logBuffer += `\n<span class="text-slate-500">${timestamp}</span> ${prefix} ${message}`;
    
    // Limit lines
    const lines = logBuffer.split('\n');
    if (lines.length > MAX_LOG_LINES) {
      logBuffer = lines.slice(-MAX_LOG_LINES).join('\n');
    }
    
    if (terminalEl) {
      terminalEl.innerHTML = logBuffer;
      terminalEl.scrollTop = terminalEl.scrollHeight;
    }
  }
  
  // Subscribe to terminal logs
  const unsubscribeTerminal = onTerminalLog((log) => {
    if (log.type === 'buffer') {
      // Initial buffer load
      logBuffer = '';
      log.errors.forEach(err => {
        const time = new Date(err.timestamp).toLocaleTimeString();
        appendLog(`${err.message} (${err.source}:${err.lineno})`, 'error');
      });
    } else if (log.type === 'error_captured') {
      appendLog(`CAPTURED: ${log.error.message} at ${log.error.source}:${log.error.lineno}`, 'error');
    } else if (log.type === 'healing_started') {
      appendLog('ANALYZING VIA NEMOTRON... Sending stack trace + codebase context...', 'ai');
      statusEl?.classList.add('animate-pulse');
      statusEl?.classList.remove('bg-emerald-500');
      statusEl?.classList.add('bg-sky-500');
    } else if (log.type === 'analyzing') {
      appendLog(log.message, 'ai');
    } else if (log.type === 'ai_response') {
      appendLog(`AI RESPONSE RECEIVED (${log.service}: ${log.model})`, 'ai');
    } else if (log.type === 'action_parsed') {
      appendLog(`ACTION: ${log.action.action} on ${log.action.targetFile || 'state'}`, 'healing');
    } else if (log.type === 'executing') {
      appendLog(`EXECUTING ${log.action} on ${log.target}...`, 'healing');
    } else if (log.type === 'github_commit_success') {
      appendLog(`GITHUB COMMIT SUCCESS: ${log.file} created/updated`, 'success');
    } else if (log.type === 'state_mutation_success') {
      appendLog(`STATE MUTATION SUCCESS: ${log.target}`, 'success');
    } else if (log.type === 'healing_completed') {
      appendLog('AUTO-HEALING COMMIT APPLIED — System restored to stable state', 'success');
      statusEl?.classList.remove('animate-pulse', 'bg-sky-500');
      statusEl?.classList.add('bg-emerald-500');
    } else if (log.type === 'healing_failed') {
      appendLog(`HEALING FAILED: ${log.error}`, 'error');
      statusEl?.classList.remove('animate-pulse', 'bg-sky-500');
      statusEl?.classList.add('bg-rose-500');
    } else if (log.type === 'health_check_started') {
      appendLog('RUNNING SYSTEM HEALTH CHECK...', 'system');
    } else if (log.type === 'health_check_completed') {
      appendLog('HEALTH CHECK COMPLETED:', 'system');
      log.checks.forEach(check => {
        const statusIcon = check.status === 'ok' || check.status === 'configured' || check.status === 'supported' || check.status === 'online' ? '✓' : '✗';
        const statusColor = check.status === 'ok' || check.status === 'configured' || check.status === 'supported' || check.status === 'online' ? 'text-emerald-400' : 'text-rose-400';
        appendLog(`  ${statusIcon} ${check.name}: <span class="${statusColor}">${check.status}</span> ${check.detail ? '- ' + check.detail : ''}`, 'system');
      });
    } else if (log.type === 'config_changed') {
      appendLog(`Auto-healer ${log.enabled ? 'ENABLED' : 'DISABLED'}`, log.enabled ? 'success' : 'warn');
    }
  });
  
  // Terminal controls
  clearBtn?.addEventListener('click', () => {
    logBuffer = '<span class="text-sky-400">[SYSTEM]</span> Terminal cleared.';
    terminalEl.innerHTML = logBuffer;
  });
  
  toggleEl?.addEventListener('change', (e) => {
    setAutoHealerEnabled(e.target.checked);
  });
  
  healthCheckBtn?.addEventListener('click', async () => {
    healthCheckBtn.disabled = true;
    healthCheckBtn.innerHTML = `<svg class="animate-spin w-4 h-4" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" fill="none"/></svg> Running...`;
    try {
      await runHealthCheck();
    } finally {
      healthCheckBtn.disabled = false;
      healthCheckBtn.innerHTML = `<svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg> Run System Health Check & Auto-Heal`;
    }
  });
  
  exportLogsBtn?.addEventListener('click', () => {
    const blob = new Blob([logBuffer.replace(/<[^>]*>/g, '')], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `autohealer-logs-${new Date().toISOString().slice(0,19).replace(/:/g, '-')}.txt`;
  ...
  // ---- Image Library
  document.getElementById('img-lib-upload')?.addEventListener('click', () => {
    document.getElementById('img-lib-file')?.click();
  });
  document.getElementById('img-lib-file')?.addEventListener('change', async (e) => {
    ...
    try {
      await addImageToLibrary(file, name);
      showToast('Image added to library', 'success');
      refreshImageLibraryGrid();
    } catch (err) {
      console.error(err);
  ...
  // Image library delete (delegated)
  document.getElementById('img-lib-grid')?.addEventListener('click', (e) => {
    const delBtn = e.target.closest('[data-img-del]');
    if (!delBtn) return;
    if (!confirm('Delete this image from library?')) return;
    removeImageFromLibrary(delBtn.dataset.imgDel);
  ...
  document.getElementById('ai-key-toggle')?.addEventListener('click', () => {
    const t = document.getElementById('ai-apikey'); if (!t) return;
    t.type = t.type === 'password' ? 'text' : 'password';
  });

  document.getElementById('ai-save-key')?.addEventListener('click', () => {
    const k = document.getElementById('ai-apikey').value.trim();
    if (!k) { showToast('API key cannot be empty', 'warn'); return; }
    setApiKey(k);
  ...
  document.querySelectorAll('[data-doc-del]').forEach(b => b.addEventListener('click', () => {
    ...
    showToast('Doctor deleted — Saved & Synced with GitHub', 'success');
    rerender();
  }));
    ...
    onMount: (root) => {
      ...
      root.querySelector('#doc-form').addEventListener('submit', (e) => {
        ...
        if (doctor?.id) {
          ...
          showToast(`Doctor updated — Saved & Synced with GitHub`, 'success');
        } else {
          addDoctor(payload);
          showToast(`Doctor added — Saved & Synced with GitHub`, 'success');
        }
        hideModal();
        rerender();
      });
...
function renderImageLibrary() {
  ...
  if (images.length === 0) {
    return `
      <div class="col-span-full py-12 text-center text-sm text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-800 border border-dashed border-slate-300 dark:border-slate-700 rounded-2xl">
        No images in library yet. Click "Upload Image" to add some.
      </div>
    `;
  }
  return images.map(img => `
    <div class="relative group bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-2 flex flex-col items-center">
      <div class="w-full aspect-square rounded-lg bg-slate-100 dark:bg-slate-900 overflow-hidden mb-2 relative">
        <img src="${escapeAttr(img.dataUrl)}" alt="${escapeAttr(img.name)}" class="w-full h-full object-cover"/>
      </div>
      <div class="text-xs font-medium text-center truncate w-full">${escapeHtml(img.name)}</div>
      <div class="text-[10px] text-slate-500 dark:text-slate-400 text-center truncate w-full">${img.size ? (img.size / 1024).toFixed(1) + ' KB' : ''}</div>
      <button data-img-del="${escapeAttr(img.id)}" class="absolute top-1 right-1 w-6 h-6 rounded-full bg-rose-500/90 text-white opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center" title="Delete">
...
function refreshImageLibraryGrid() {
  const grid = document.getElementById('img-lib-grid');
  if (grid) grid.innerHTML = renderImageLibrary();
}