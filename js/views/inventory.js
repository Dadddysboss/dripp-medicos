...
import {
  CATEGORIES, PACK_TYPES, LOW_STOCK_BASE_THRESHOLD,
...
const filters = {
  query: '',
  category: '',
  tab: 'all',     // 'all' | 'cold' | 'controlled' | 'low' | 'expired'
};
...
export function getProductStatus(product) {
  ...
  // Low stock check (in base units; also flag if ≤ 10 boxes)
  const base = Math.max(0, parseInt(product?.totalBaseUnits, 10) || 0);
  const { tabletsPerBox } = splitBaseUnits(base, product);
  if (base <= LOW_STOCK_BASE_THRESHOLD) result.isLowStock = true;
...
function renderRow(p) {
  ...
  return `
    <tr class="${rowTint} hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
      ...
      <td class="px-4 py-3 align-top text-xs">${escapeHtml(p.expiryDate || '—')}</td>
...
function stockAdjustByBox(id, deltaBoxes) {
  ...
  try {
    ...
    showToast(`Stock: ${p.name} (${deltaBoxes > 0 ? '+' : ''}${deltaBoxes} box${Math.abs(deltaBoxes)===1?'':'es'}) → ${next} base units`, 'success');
    rerenderTable();
...
function handleFormSubmit(existingProduct, root) {
  ...
  try {
    if (existingProduct?.id) {
      ...
      showToast(`${name} updated & synced to GitHub`, 'success');
    } else {
      addProduct(payload);
      showToast(`${name} added & synced to GitHub`, 'success');
    }
    hideModal();
    rerenderTable();
  } catch (err) {
  ...
  `, {
    ...
    onMount: (root) => {
      ...
      root.querySelector('[data-confirm]')?.addEventListener('click', () => {
        try {
          deleteProduct(id);
          showToast(`${p.name} deleted from inventory`, 'success');
          hideModal();
          rerenderTable();
        } catch (err) {
          console.error(err);
