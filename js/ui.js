...
// ============================================================
// 4. Nav active highlighting (DOM-safe)
// ============================================================
...
export function openModal(html, { onMount, size = 'md' } = {}) {
  ...
  root.innerHTML = `
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" data-modal-backdrop>
      <div class="w-full ${sizeCls} ${responsiveCls} bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-fadeUp flex flex-col">
        <div class="flex-shrink-0 px-4 py-3 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div></div>
          <button data-modal-close class="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 touch-target" aria-label="Close modal">
  ...
  // Handle [data-close] buttons inside modal content (legacy support)
  ...
  // Get the modal content container (where the HTML is injected)
  const modalContent = root.querySelector('.flex-1.overflow-y-auto');
  
  if (typeof onMount === 'function') {
    setTimeout(() => onMount(modalContent), 0);
  }
}
...
// ============================================================
// 8. Hash router (DOM-safe, robust error handling)
// ============================================================
...
export function createRouter(routes) {
  async function render() {
    ...
    // Update nav highlight and page title safely
    highlightNav(view);

    const container = document.getElementById('view-container');
    if (!container) {
      console.warn('view-container not found, skipping render');
      return;
    }

    // Show subtle loading only for initial load, not navigation
    const isInitialLoad = !container.querySelector('.view');
    if (isInitialLoad) {
    ...
    try {
      ...
      console.error(`Router render error for ${view}:`, e);
      container.innerHTML = `
        <div class="p-6 text-center text-rose-600 dark:text-rose-400">
          ...
          <h3 class="font-semibold">Failed to load view</h3>
          <p class="text-sm mt-1">${escapeHtml(e?.message || String(e))}</p>
          <button onclick="location.reload()" class="mt-4 px-4 py-2 bg-brand-600 text-white rounded-lg hover:bg-brand-700">Reload App</button>
        </div>
      `;
    }
  }
